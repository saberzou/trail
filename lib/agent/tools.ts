...
const defaultLookupAll: LookupAllFn = async (host) => {
  ...
  // of them point at a blocked range.
...
/**
 * Return true when an IPv6 literal is loopback, link-local, ULA, or an
 * IPv4-mapped form of a blocked v4 range.
 */
...
// redirects. Lowercased; we'll compare case-insensitively.
  ...
  return tool({
    ...
    execute: async ({ url }) => {
      ...
      for (let hop = 0; hop <= maxRedirects; hop++) {
        ...
        let res: Response;
        try {
          res = await f(currentUrl.toString(), {
            method: "GET",
            headers,
            redirect: "manual",
            signal,
          ...
          // dispatcher will be GC'd; explicit close is best-effort.
      ...
      const html = await readCappedBody(finalResponse, cap);
