...
/**
 * Extract a share-preview image URL from page HTML — the same metadata
 * chat apps use to unfurl links. Checks `og:image` (+ `og:image:secure_url`)
 * then `twitter:image` as fallback. Handles both attribute orders and both
 * `property=`/`name=` forms. Relative URLs are resolved against `baseUrl`;
 * non-http(s) results are rejected. Returns null when nothing usable.
 *
 * Regex-over-HTML is fine here: og meta tags live in <head>, are flat
 * single-tag matches, and we never interpret nesting.
 */
export function extractPreviewImage(html, baseUrl) {
  ...
  for (const name of names) {
    // <meta property="og:image" content="..."> and content-first order.
    ...
    if (!m) continue;
    try {
      const resolved = new URL(m[1].trim(), baseUrl);
      if (resolved.protocol === "http:" || resolved.protocol === "https:") {
        return resolved.toString();
      }
    } catch {
      // unparseable content value — try the next candidate
    }
  }
  return null;
}
