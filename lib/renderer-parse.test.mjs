...
describe("extractPreviewImage", () => {
  ...
  it("extracts og:image (property= form)", () => {
    const html = `<head><meta property="og:image" content="https://cdn.example.com/hero.png"></head>`;
    expect(extractPreviewImage(html, BASE)).toBe(
      "https://cdn.example.com/hero.png",
    );
  });

  it("extracts og:image with name= and content-first attribute order", () => {
    const html = `<meta content="https://cdn.example.com/a.jpg" name="og:image">`;
    expect(extractPreviewImage(html, BASE)).toBe(
      "https://cdn.example.com/a.jpg",
    );
  });

  it("prefers og:image:secure_url over og:image, falls back to twitter:image", () => {
    const both = `<meta property="og:image" content="http://x.com/a.png"><meta property="og:image:secure_url" content="https://x.com/secure.png">`;
    expect(extractPreviewImage(both, BASE)).toBe("https://x.com/secure.png");
    const twitterOnly = `<meta name="twitter:image" content="https://x.com/tw.png">`;
    expect(extractPreviewImage(twitterOnly, BASE)).toBe("https://x.com/tw.png");
  });

  it("resolves relative URLs against the page URL", () => {
    const html = `<meta property="og:image" content="/img/hero.webp">`;
    expect(extractPreviewImage(html, BASE)).toBe(
      "https://example.com/img/hero.webp",
    );
  });

  it("rejects non-http(s) schemes and returns null when absent", () => {
    expect(
      extractPreviewImage(
    ...
    expect(
      extractPreviewImage("<head><title>No meta</title></head>", BASE),
    ).toBeNull();
    expect(extractPreviewImage("", BASE)).toBeNull();
  });
});
